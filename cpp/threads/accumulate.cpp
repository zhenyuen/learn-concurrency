...
T parallel_accumulate(Iterator first, Iterator last, T init) {
    ...
    std::vector<std::jthread> threads(n_threads);
    std::vector<std::future<T>> futures(n_threads);

    Iterator block_start = first, block_end = first;

    for (int i = 0; i < n_threads; i++) {
        std::advance(block_end, block_size);
        auto functor = accumulate_block<Iterator, T>();
        std::packaged_task<T(Iterator, Iterator)> task(functor);
        futures[i] = task.get_futures();
        threads[i] = std::jthread(std::move(functor), block_start, block_end, std::ref(results[i]));
